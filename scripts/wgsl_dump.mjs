...
// Screenshots: web/ws_NN.png. Console → /tmp/wgsl_dump.log.
import { spawn } from "node:child_process";
import { createWriteStream } from "node:fs";
import { chromium } from "playwright";
const srv = spawn("node", ["scripts/serve.mjs", "web", "8151"], { stdio: "ignore" });
...
const full = createWriteStream("/tmp/wgsl_dump.log");
let errCount = 0;
await page.addInitScript(() => { window.__CAPENV = { WGPU_DUMP_WGSL_STDERR: "deferred_combine" }; });
page.on("console", (m) => full.write(m.text() + "\n"));
page.on("pageerror", (e) => { errCount++; full.write("PAGEERROR:\n" + (e.stack || e.message) + "\n"); });
await page.goto("http://localhost:8151/blender-gui.html", { waitUntil: "load" });
for (let i = 0; i < 70; i++) {
...
const names = ["layout", "modeling", "sculpting", "uv", "texpaint", "shading"];
for (let i = 0; i < names.length; i++) {
  ...
  await settle(5000);
  await page.locator("#canvas").screenshot({ path: `web/ws_${String(i).padStart(2, "0")}_${names[i]}.png` });
