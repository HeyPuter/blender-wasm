...
// Screenshots: web/mb_NN.png. Console → /tmp/modeling_boot_test.log.
import { spawn } from "node:child_process";
import { createWriteStream } from "node:fs";
import { chromium } from "playwright";
const srv = spawn("node", ["scripts/serve.mjs", "web", "8163"], { stdio: "ignore" });
...
const full = createWriteStream("/tmp/modeling_boot_test.log");
let errCount = 0;
...
await page.goto("http://localhost:8163/blender-gui.html", { waitUntil: "load" });
for (let i = 0; i < 70; i++) {
...
await page.setViewportSize({ width: 1600, height: 951 });
await new Promise(r => setTimeout(r, 3000));
await page.locator("#canvas").screenshot({ path: "web/mb_h951.png" });
await page.setViewportSize({ width: 1600, height: 950 });
await new Promise(r => setTimeout(r, 3000));
await page.locator("#canvas").screenshot({ path: "web/mb_h950.png" });
