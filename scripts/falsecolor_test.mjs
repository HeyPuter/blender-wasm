...
// Screenshots: web/fc_NN.png. Console → /tmp/falsecolor_test.log.
import { spawn } from "node:child_process";
import { createWriteStream } from "node:fs";
import { chromium } from "playwright";
const srv = spawn("node", ["scripts/serve.mjs", "web", "8179"], { stdio: "ignore" });
...
const full = createWriteStream("/tmp/falsecolor_test.log");
let errCount = 0;
...
await page.goto("http://localhost:8179/blender-gui.html", { waitUntil: "load" });
for (let i = 0; i < 70; i++) {
...
for (let i = 0; i < names.length; i++) {
  ...
  await settle(5000);
  await page.locator("#canvas").screenshot({ path: `web/fc_${String(i).padStart(2, "0")}_${names[i]}.png` });
