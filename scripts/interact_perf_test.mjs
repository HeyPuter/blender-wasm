...
// movement, WGPU_STATS per-frame telemetry. Console → /tmp/interact_perf.log.
import { spawn } from "node:child_process";
import { createWriteStream } from "node:fs";
import { chromium } from "playwright";
const srv = spawn("node", ["scripts/serve.mjs", "web", "8169"], { stdio: "ignore" });
...
const full = createWriteStream("/tmp/interact_perf.log");
...
await page.goto("http://localhost:8169/blender-gui.html", { waitUntil: "load" });
for (let i = 0; i < 70; i++) {
