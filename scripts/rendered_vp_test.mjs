...
// Screenshots: web/rv_NN.png. Console → /tmp/rendered_vp_test.log.
import { spawn } from "node:child_process";
import { createWriteStream } from "node:fs";
import { chromium } from "playwright";
const srv = spawn("node", ["scripts/serve.mjs", "web", "8159"], { stdio: "ignore" });
...
const full = createWriteStream("/tmp/rendered_vp_test.log");
let errCount = 0;
await page.addInitScript(() => { window.__CAPENV = {};
  window.__BARGS = ["--python-expr", "import bpy\n[setattr(a.spaces[0].shading,'type','RENDERED') for w in bpy.data.workspaces for s in w.screens for a in s.areas if a.type=='VIEW_3D']"]; });
page.on("console", (m) => full.write(m.text() + "\n"));
page.on("pageerror", (e) => { errCount++; full.write("PAGEERROR:\n" + (e.stack || e.message) + "\n"); });
await page.goto("http://localhost:8159/blender-gui.html", { waitUntil: "load" });
for (let i = 0; i < 70; i++) {
...
for (let i = 0; i < names.length; i++) {
  ...
  await settle(5000);
  await page.locator("#canvas").screenshot({ path: `web/rv_${String(i).padStart(2, "0")}_${names[i]}.png` });
